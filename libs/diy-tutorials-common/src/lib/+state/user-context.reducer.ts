...
export const userContextReducer = createReducer(initialUserContextState, {
    ...
    [TutorialActionTypes.ShowProductsForProductRange]: (state: UserContextState, action: ShowProductsForProductRange) => {
        const {productRangeUUID} = action.payload;
        state.expandedProductRanges = state.displayedProducts || {};
        state.expandedProductRanges[productRangeUUID] = true;
        return state;
    },
    [TutorialActionTypes.ToggleShowProductsForAllProductRanges]: (state: UserContextState, action: AnyAction) => {
        state.expandAllProductRanges = !state.expandAllProductRanges;
        return state;
    },
});
